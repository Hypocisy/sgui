...
public class SGuiTest {
		...
		try {
			...
			BookGui gui = new BookGui(player, bookBuilder) {
				...
				public boolean onCommand(String command) {
					...
					bookBuilder.addPage(Component.literal(command));
			...
			MerchantGui gui = new MerchantGui(player, false) {
				...
				public void onSuggestSell(MerchantOffer offer) {
					if (offer != null && offer.getResult() != null) {
						...
						offer.getResult().set(DataComponents.CUSTOM_NAME, ((MutableComponent) player.getName()).append(Component.literal("'s ")).append(offer.getResult().getDisplayName()));
						this.sendUpdate();
					}
				}
			};
		...
		try {
			...
			BookInputGui gui = new BookInputGui(player) {
				...
				public void onBookWritten(@Nullable String title, List<String> pages, boolean signed) {
					this.player.sendSystemMessage(Component.literal("Title was: " + title), false);
					this.player.sendSystemMessage(Component.literal("Page 0 was: " + pages.get(0)), false);
					this.player.sendSystemMessage(Component.literal("Is signed: " + signed), false);
	...
	private static int test10(CommandContext<CommandSourceStack> objectCommandContext) {
		try {
			...
			gui.setSlot(7, new GuiElementBuilder()
					...
					.glow()
					.setName(Component.literal("Bye")
							.setStyle(Style.EMPTY.withItalic(false).withBold(true).applyFormats(ChatFormatting.RED)))
					.addLoreLine(Component.literal("Some lore"))
					.addLoreLine(Component.literal("More lore").withStyle(ChatFormatting.RED))
					.setCount(3)
					.setCallback((index, clickType, actionType) -> gui.close())
	...
	public static void onRegisterCommands(RegisterCommandsEvent event) {
		...
		dispatcher.register(
				Commands.literal("test").executes(SGuiTest::test)
		);
		dispatcher.register(
				Commands.literal("test2").executes(SGuiTest::test2)
		);
		dispatcher.register(
				Commands.literal("test3").executes(SGuiTest::test3)
		);
		dispatcher.register(
				Commands.literal("test4").executes(SGuiTest::test4)
		);
		dispatcher.register(
				Commands.literal("test5").executes(SGuiTest::test5)
		);
		dispatcher.register(
				Commands.literal("test6").executes(SGuiTest::test6)
		);
		dispatcher.register(
				Commands.literal("test7").executes(SGuiTest::test7)
		);
		dispatcher.register(
				Commands.literal("test8").executes(SGuiTest::test8)
		);
		dispatcher.register(
				Commands.literal("test9").executes(SGuiTest::test9)
		);
		dispatcher.register(
				Commands.literal("test10").executes(SGuiTest::test10)
		);
		dispatcher.register(
				Commands.literal("test11").executes(SGuiTest::test11)
		);
		dispatcher.register(
				Commands.literal("test12").executes(SGuiTest::test12)
		);
		dispatcher.register(
				Commands.literal("test13").executes(SGuiTest::test13)
		);
		dispatcher.register(
				Commands.literal("snake").executes(SGuiTest::snake)
		);
	}
}
