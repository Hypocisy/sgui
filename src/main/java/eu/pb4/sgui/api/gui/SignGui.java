...
public class SignGui implements GuiInterface {
	...
	public SignGui(ServerPlayer player) {
		this.player = player;
		this.signEntity = new VirtualSignBlockEntity(player.level(), new BlockPos(player.getBlockX(), Math.min(player.level().getHeight() - 1, player.blockPosition().getY() + 5), player.blockPosition().getZ()), Blocks.OAK_SIGN.defaultBlockState());
	}
