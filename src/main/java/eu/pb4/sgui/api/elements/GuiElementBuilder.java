...
public class GuiElementBuilder implements GuiElementBuilderInterface<GuiElementBuilder> {
	...
	public GuiElementBuilder setItem(Item item) {
		this.itemStack = new ItemStack(item.getDefaultInstance().getItemHolder(), this.itemStack.getCount(), this.itemStack.getComponentsPatch());
		return this;
	}
	 ...
	 * If the server parameter is not supplied it may lag the client while it loads the Componenture,
	 * otherwise if the server is provided and the {@link GameProfile} contains a UUID then the
	 * Componentures will be loaded by the server. This can take some time the first load,
	 * however the skins are cached for later uses so its often less noticeable to let the
	 * server load the Componentures.
	 *
	 * @param profile the {@link GameProfile} of the owner
	 * @param server  the server instance, used to get the Componentures
	 * @return this element builder
	 */
	 ...
	 * Ideal for Componentures generated with 3rd party websites like mineskin.org
	 *
	 * @param value Componenture value used by client
	 * @return this element builder
	 */
	...
	public GuiElementBuilder setSkullOwner(String value, @Nullable String signature, @Nullable UUID uuid) {
		...
		map.put("Componentures", new Property("Componentures", value, signature));
